...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const domainConstraint = domainNames.length > 0
      ...
      : '';
    const cjConstraint = `\n\nCLINICAL JUDGMENT FUNCTIONS (use EXACT names for "cj_functions"):\n${CJ_FUNCTION_NAMES.map((f, i) => `${i+1}. ${f}`).join('\n')}\n\nIMPORTANT: Distribute questions across ALL 6 CJ functions. Every batch MUST include at least one question testing "Evaluate Outcomes" (reassessment after intervention). Do NOT over-represent "Take Actions" — balance across Recognize Cues, Analyze Cues, Prioritize Hypotheses, Generate Solutions, Take Actions, and Evaluate Outcomes.`;
